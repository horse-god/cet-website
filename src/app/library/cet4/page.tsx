...
export const metadata: Metadata = {
  title: "四级真题",
  description: "2019-2025 年英语四级真题试卷下载，含答案和听力音频。",
};

export const dynamic = "force-static";

export default function CET4Page() {
  const exams = getExamsByLevel("cet4");
  return <ExamList exams={exams} level="cet4" title="英语四级 CET-4" count={exams.length} />;
}

function ExamList({ exams, level, title, count }: { exams: ExamPaper[]; level: string; title: string; count: number }) {
    ...
    <div className="animate-fade-in py-10">
      <div className="container mx-auto max-w-5xl px-4">
        <div className="text-center mb-10">
          ...
          <p className="text-muted text-lg">{count} 套真题 · 含试卷 PDF + 答案 + 听力 MP3</p>
        </div>
            ...
            return (
              <div key={year} className="bg-white dark:bg-slate-800 rounded-2xl border border-border overflow-hidden">
                ...
                <div className="divide-y divide-border">
                  {papers.map((paper) => (
                    <Link key={paper.id} href={`/library/cet4/${paper.id}`} className="flex items-center justify-between px-5 py-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 dark:bg-slate-800/50 transition-colors group">
